...
def inference_model(model_name,lora_adapter=None,system_prompt="",user_prompt="",follow_up="",override=False,include_supp=False):
        ...
        if include_supp == True:
            dataset = prepare_dataset_for_inference(df=df,text_col="PublicNarrative",class_col="BHR_type",sample_size=297,supp_columns=["Supp","OfficerNarrative"],leading_columns=["Offense"])
        ...
        for i, text in enumerate(texts):
            concatenated_prompt = system_prompt+" " + user_prompt + " " + text + " " + follow_up
...
user_prompt = """[INST]As the police department's premier virtual assistant, your task is to analyze the following narratives to determine current or future behavioral health indicators, independent of any danger or threat. Each narrative should be tagged with any ofof the following Behavioral Health subcategories: "Domestic Social", "NonDomestic Social", "Mental Health", or "Substance Abuse". non-Behavioral Health related narratives should be classified as "Other" Note that there are no boundaries on predicting potential issues; any relevant signals should be appropriately classified. Begin your response using the format: <Tag> [Answer] </Tag>. Proceed with the text classification: [/INST]\n[INST]
"""
...
system_prompt = """[INST]Behvaioral Health Classification Overview:\n
                ...
                ##It is not possible for "Other" and any of the Behavioral Health categories to exist simultaneously
                Objective of Classification:\n
                The Primary goal of this classification system is not only to manage existing crises but also to identify potential future behavioral health crises, thereby enabling earlier and more effective interventions[/INST]
                """
lora_adapter = r"/home/wstigall/workspace/Mistral-HELPR-v3"
inference_model(model_name="mistralai/Mistral-7B-Instruct-v0.2",lora_adapter=None,system_prompt=system_prompt,follow_up=follow_up,user_prompt=user_prompt,include_supp=True,override=True)
#inference_model(model_name="mistralai/Mixtral-8x7B-Instruct-v0.1",lora_adapter=None,system_prompt=system_prompt,use_class_definitions=True,override=True)
